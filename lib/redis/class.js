...
module.exports = SuperJS.Api.Adapter.extend({
  ...
  set: function(resolve, reject, key, value, connectionName) {
    ...
    client.set(key, value, function(err, reply) {
      ...
      if( err ) {
        reject(new SuperJS.Error('redis_error', 'An error occurred saving data to the `' + connectionName + '` connection.'));
      } else {
        resolve(reply);
      }
  ...
  get: function(resolve, reject, key, connectionName) {
    ...
    client.get(key, function(err, reply) {
      ...
      if( err ) {
        reject(new SuperJS.Error('redis_error', 'An error occurred retreiving data from the `' + connectionName + '` connection.', err));
      } else {
        resolve(reply);
      }
  ...
  del: function(resolve, reject, key, connectionName) {
    ...
    client.del(key, function(err, reply) {
      ...
      if( err ) {
        reject(new SuperJS.Error('redis_error', 'An error occurred deleting data from the `' + connectionName + '` connection.', err));
      } else {
        resolve(reply);
      }
  ...
  expire: function(resolve, reject, key, seconds, connectionName) {
    ...
    client.expire(key, seconds, function(err, reply) {
      ...
      if( err ) {
        reject(new SuperJS.Error('redis_error', 'An error occurred setting the expiration of `' + key + '` on the `' + connectionName + '` connection.', err));
      } else {
        resolve(reply);
      }
